...
function write(tracks, options, fileName) {
  ...
  Object.keys(tracks).forEach(name => {
    ...
    function saveTrack(suffix) {
      ...
      files.push({
        filepath,
        settings: _tracks.reduce((prev, cur) =>
          utils.merge(prev, cur)
          , {}),
      });
    }
...
module.exports = ast => {
  ...
  Object.keys(ast.tracks).forEach(key => {
    ...
    const prop = key.indexOf('>') > -1
      ? key.split('>')[0].trim()
      : key;

    if (!ast.tracks[prop]) {
      throw new Error(`Expecting a valid track to extend, given '${key}'`);
    }
    ...
    map[prop].push({
      track: key.indexOf('>') > -1
        ...
        : null,
      clipObj: clip,
      settings: utils.merge(ast.tracks[key].settings, ast.tracks[prop].settings),
    });
  });
